...
void Tracker::addEntry(unsigned long timestamp, int slope, std::vector<int> humidities, std::vector<int> illuminations, std::vector<int> temperatures, int duration,
                       std::set<int> species, QTemporaryDir & tmp_data_dir)
{
    EntryData entry(timestamp, slope, humidities, illuminations, temperatures, species, duration);
